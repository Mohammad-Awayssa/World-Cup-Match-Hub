...
export function HomeMatchRow({ match, compact = false }) {
  ...
  return (
    <article className={`match-row grid items-center gap-3 px-4 py-4 sm:px-6 ${isFinished ? 'match-row-finished' : ''} ${compact ? 'lg:grid-cols-[150px_1fr_150px_24px]' : 'lg:grid-cols-[120px_1fr_180px]'}`}>
      <div className="border-white/10 lg:border-r">
        <strong className={`block text-sm font-bold ${isLive ? 'text-red-400' : isFinished ? 'text-slate-300' : 'text-white'}`}>
          {isLive ? 'LIVE' : isFinished ? 'Finished' : formatLocalTime(match.kickoffUTC)}
        </strong>
        <span className={`mt-1 block text-xs ${isFinished ? 'text-slate-500' : 'text-neon'}`}>{match.group ? `Group ${match.group}` : match.stage}</span>
